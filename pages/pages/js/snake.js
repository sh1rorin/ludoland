...
var factor = parseFloat(window.getComputedStyle(canvas).width) / 640;
...
window.addEventListener("resize",() => {
  ...
  factor = parseFloat(window.getComputedStyle(canvas).width) / 640;
...
function mouseuphandler(e){
  ...
  if (!run){
  	if (!if_title && canvas.width/4 - 75 < x && x < canvas.width/4 +135 && canvas.height/2 < y && y < canvas.height/2 + 105){
  		signal = "retry";
  	} else if (!if_title && canvas.width*3/4 - 130 < x && x < canvas.width*3/4 + 80 && canvas.height/2 < y && y < canvas.height/2 + 105){
  		signal = "title";
      if_title = true;
  	} else if(if_title){
    	run = true;
